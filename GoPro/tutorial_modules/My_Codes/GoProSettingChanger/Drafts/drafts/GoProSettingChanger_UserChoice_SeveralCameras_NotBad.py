...
# Main function to connect to GoPro devices and change FPS
async def main(identifier: str | None, timeout: int | None) -> None:
    ...
    if fps == 60:
        ...
        request = bytes([0x03, 0x03, 0x01, 0x02])
    elif fps == 120:
        logger.info("Setting the fps to 120")
        request = bytes([0x03, 0x03, 0x01, 0x01])
    elif fps == 240:
    ...
    for device in matched_devices:
        try:
            ...
            logger.debug(f"Writing to {GoProUuid.SETTINGS_REQ_UUID}: {request.hex(':')}")
            event.clear()
            await client.write_gatt_char(GoProUuid.SETTINGS_REQ_UUID.value, request, response=True)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Connect to GoPro cameras and change their FPS.")
