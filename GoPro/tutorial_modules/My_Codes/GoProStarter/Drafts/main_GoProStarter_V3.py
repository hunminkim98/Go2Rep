...
async def start_recording(clients: List[BleakClient]) -> None:
    ...
    for client in clients:
        Start_human_readable_time= await client.write_gatt_char_with_TimeStamp(COMMAND_REQ_UUID, command, response=True)
        #logger.info(f"Starting recording on {camera_names[client.address]} at {Start_human_readable_time}")   
...
async def stop_recording(clients: List[BleakClient]) -> None:
    ...
    for client in clients:
        Stop_human_readable_time=await client.write_gatt_char_with_TimeStamp(COMMAND_REQ_UUID, command, response=True)
        #logger.info(f"Starting recording on {camera_names[client.address]} at {Stop_human_readable_time}")
