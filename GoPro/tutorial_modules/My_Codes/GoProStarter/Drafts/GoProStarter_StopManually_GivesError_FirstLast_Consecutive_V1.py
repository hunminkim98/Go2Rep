...
async def start_recording(clients: List[BleakClient]) -> None:
    ...
    for client in clients:
        ...
        logger.info(f"Starting recording on {camera_names[client.address]} at {start_times[client.address]:.6f} sec")
...
async def stop_recording(clients: List[BleakClient]) -> None:
    ...
    for client in clients:
        ...
        logger.info(f"Stopping recording on {camera_names[client.address]} at {stop_times[client.address]:.6f} sec")
        await client.write_gatt_char(COMMAND_REQ_UUID, command, response=True)
