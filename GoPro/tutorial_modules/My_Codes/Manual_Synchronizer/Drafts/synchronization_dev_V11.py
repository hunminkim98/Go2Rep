...
def navigate_frames(video_capture, win_name="Select Reference Frame", start_frame=0, end_frame=None):
    global frame_number, total_frames, video, window_name
    if video_capture is None:
        print("❌ Error: Video is not loaded correctly.")
        return None
...
def synchronize_videos(trial_name, video_paths):
    ...
    for video_path in video_paths:
        video = open_video(video_path)
        if video:
            offset = navigate_frames(video, f"Sync {os.path.basename(video_path)}", start_frame, end_frame)
