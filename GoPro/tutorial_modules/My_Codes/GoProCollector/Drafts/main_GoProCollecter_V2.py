...
def prompt_user_for_date(media_data):
    ...
    while True:
        try:
            ...
            if 1 <= choice <= len(unique_dates):
                return unique_dates[choice - 1]  # Return only the selected date (not time)
            else:
...
def download_selected_media():
    ...
    selected_date = prompt_user_for_date(media_files)

    # Match videos based only on the selected date
