        ...
        if use_sync_file and sync_json:
            ...
            if trial_id in sync_json:
                ...
                for clip, video_path, _ in video_data:
                    ...
                    video_out_name = re.sub(r'.*-(CAMERA\d+)-.*', r'\1', video_name)
                    output_path = os.path.join(camera_folder, f"{video_out_name}.{format_choice}" if convert else f"{video_out_name}.avi")

                    fourcc = cv2.VideoWriter_fourcc(*'XVID') if format_choice == 'avi' else cv2.VideoWriter_fourcc(*'mp4v')
            ...
            for clip, video_path, _ in video_data:
                ...
                camera_folder = os.path.join(trial_target_dir, camera_name)
                os.makedirs(camera_folder, exist_ok=True)
                output_name = re.sub(r'[^\\]+-(CAMERA\d+)-[^\\]+\.mp4$', r'\1', video_name)
                ...
                while cap.isOpened():
                    ret, frame = cap.read()
                    if not ret or kept_frames >= (min_frames // downsample_rate if downsample else min_frames):
                        break
                    if not downsample or frame_idx % downsample_rate == 0:
                        out.write(frame)
                        kept_frames += 1
