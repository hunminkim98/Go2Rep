...
def process_videos(source_dir, target_dir, sync_json=None, convert=False, format_choice='mp4'):
    ...
    for trial_index, trial in enumerate(trials, start=1):
        ...
        downsample = input(f"Do you want to downsample the videos (which decreases the theia calculation time for calibration trials) in this trial? (yes/no): ").lower() == 'yes'
        downsample_rate = 1
        if downsample:
            while True:
                try:
        ...
        if use_sync_file and sync_json:
            ...
            if trial_id in sync_json:
                ...
                for clip, video_path, _ in video_data:
                    ...
                    match = re.search(r'(CAMERA\d+)', video_name)
                    video_out_name = match.group(1) if match else "CAMERA_UNKNOWN"
                    output_path = os.path.join(camera_folder, f"{video_out_name}.{format_choice}" if convert else f"{video_out_name}.avi")


                    fourcc = cv2.VideoWriter_fourcc(*'avc1') if format_choice == 'avi' else cv2.VideoWriter_fourcc(*'avc1')
                    out = cv2.VideoWriter(output_path, fourcc, fps / downsample_rate if downsample else fps, (width, height))

                    frame_idx = 0
                    kept_frames = 0
                    cap.set(cv2.CAP_PROP_POS_FRAMES, start_frame)

                    max_frames = (end_frame - start_frame) // downsample_rate if downsample else (end_frame - start_frame)
                    pbar = tqdm(total=max_frames, desc=f"Writing {video_name}", unit="frame")

                    while cap.isOpened():
                        ret, frame = cap.read()
                        if not ret or frame_idx > (end_frame - start_frame):
                            break
                        if not downsample or frame_idx % downsample_rate == 0:
                            out.write(frame)
                            kept_frames += 1
                            pbar.update(1)
                        frame_idx += 1
            ...
            for clip, video_path, _ in video_data:
                ...
                os.makedirs(camera_folder, exist_ok=True)
                match = re.search(r'(CAMERA\d+)', video_name)
                output_name = match.group(1) if match else "CAMERA_UNKNOWN"
                output_path = os.path.join(camera_folder, f"{output_name}.{format_choice}" if convert else f"{output_name}.avi")
                ...
                max_frames = min_frames // downsample_rate if downsample else min_frames
                pbar = tqdm(total=max_frames, desc=f"Writing {video_name}", unit="frame")

                while cap.isOpened():
                    ret, frame = cap.read()
                    if not ret or kept_frames >= max_frames:
                        break
                    if not downsample or frame_idx % downsample_rate == 0:
                        out.write(frame)
                        kept_frames += 1
