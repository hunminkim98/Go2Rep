...
async def provision_all_gopros(ssid: str, password: str, certificate_dir: Path, gopro_list: list[str] = None):
...
def show_wifi_selection_dialog(wifi_list):
    dialog_result = {}
    ...
    def on_submit():
        ...
        if not entered_password:
            messagebox.showerror("Error", "Please enter the Wi-Fi password.")
            return

        dialog_result["ssid"] = selected_ssid
        dialog_result["password"] = entered_password
        dialog.destroy()

    dialog = tk.Toplevel()
    dialog.title("Wi-Fi Provisioning")

    tk.Label(dialog, text="Select Wi-Fi network:").pack(pady=(10, 0))
    ssid_var = tk.StringVar()
    ssid_dropdown = ttk.Combobox(dialog, textvariable=ssid_var, values=wifi_list, state="readonly")
    ssid_dropdown.pack(padx=10, pady=5)

    tk.Label(dialog, text="Enter Wi-Fi password:").pack(pady=(10, 0))
    ...
    return dialog_result.get("ssid"), dialog_result.get("password")

async def run_provisioning_gui(cert_dir: Path, gopro_list: list[str] = None):
    # Scan Wi-Fi networks
    wifi_list = scan_wifi_networks()

    if not wifi_list:
        messagebox.showwarning("No Networks", "No Wi-Fi networks found.")
        return

    # Show unified dialog
    ssid, password = show_wifi_selection_dialog(wifi_list)
    ...
    # Run provisioning
    await provision_all_gopros(ssid, password, cert_dir, gopro_list)
    messagebox.showinfo("Done", f"Provisioning complete.\nCertificates saved in: {cert_dir}")
