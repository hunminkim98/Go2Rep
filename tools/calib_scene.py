...
def calib_calc_fun(calib_dir, intrinsics_config_dict, extrinsics_config_dict,calib_file_path,filename_convention):
    '''
    ...
    if calculate_extrinsics:
        ...
        ret, C, S, D, K, R, T = calibrate_extrinsics(calib_dir, extrinsics_config_dict, C, S, K, D, filename_convention)
    else:
...
def calibrate_extrinsics(calib_dir, extrinsics_config_dict, C, S, K, D, filename_convention):
    '''
    ...
    if filename_convention==1:
        cam_ids_in_C = [c.replace('int_cam', '') for c in C]
        id_to_ext_cam = {cam.replace('ext_cam', ''): cam for cam in extrinsics_cam_listdirs_names}
        extrinsics_cam_listdirs_names = [id_to_ext_cam[cid] for cid in cam_ids_in_C]
...
def parse_timestamp(filename, filename_convention):
    ...
    if filename_convention == 1:
        # Matches: 20250805_153045-GoPro1234-
        trial_pattern = re.compile(r"(\d{8}_\d{6})-GoPro\d+-")
    elif filename_convention == 2:
        # Matches: 20250805_153045-CAMERA01-
...
def get_camera_number(filename,filename_convention):
    """Extracts the camera number from filename based on convention."""
    if filename_convention == 1:
        # Matches: GoPro1234
        match = re.search(r"GoPro(\d+)", filename)
    elif filename_convention == 2:
        # Matches: CAMERA01
...
def group_videos_by_trial(video_files,filename_convention, time_tolerance=8):
    """Groups videos into trials allowing a ±time_tolerance seconds difference."""
    trials = []
    video_data = [(file, parse_timestamp(file,filename_convention)) for file in video_files]
...
def read_toml(toml_path):
    ...
    for cam in list(calib.keys()):
        if cam != 'metadata':
            C.append(calib[cam]['name'])
            S.append(np.array(calib[cam]['size']))
            D.append(np.array(calib[cam]['distortions']))
...
def rotate_cam(r, t, ang_x=np.pi, ang_y=0, ang_z=0):
    ...
    r_ax_h = np.block([[r_ax, np.zeros((3, 1))], [np.zeros(3), 1]])
...
def qca_write(qca_path, C, S, D, K, R, T, binning_factor, pixel_size,filename_convention):
    ...
    root = etree.Element('calibration', source=os.path.basename(qca_path), created='some_time', qtmversion='none', type='regular', wandLength='none', maximumFrames="none")
    etree.SubElement(root, 'results', stddev='0.', minmaxdiff='0.')
    cams = etree.SubElement(root, 'cameras')
    for c in range(len(C)):
        if filename_convention == 1:
            serial_list = C[c].replace('int_cam', 'GoPro')  # Example: GoPro1000, GoPro1001, ...
        elif filename_convention == 2:
            serial_list = C[c].replace('int_cam', 'CAMERA')     # Example: CAMERA01, CAMERA02, ...
        else:
            serial_list = C[c]  # fallback to original name if no match
        cam = etree.SubElement(cams, 'camera', active='1', pointcount='999999999', avgresidual='0.', serial=serial_list, model='Miqus Video Plus', viewrotation='0')
        ...
        etree.SubElement(cam, 'intrinsic', focallength=str(fm[c]),
                 ...
                 focalLengthU=str(K[c][0, 0]), focalLengthV=str(K[c][1, 1]), centerPointU=str(K[c][0, 2]), centerPointV=str(K[c][1, 2]),
                 skew='0.000000', radialDistortion1='0', radialDistortion2='0',
                 radialDistortion3='0.000000', tangentalDistortion1='0', tangentalDistortion2='0')
        # etree.SubElement(cam, 'intrinsic', focallength=str(fm[c]),
                         # sensorMinU='0.000000', sensorMaxU=str(S[c][0] * 64), sensorMinV='0.000000', sensorMaxV=str(S[c][1] * 64),
                         # focalLengthU=str(K[c][0, 0]), focalLengthV=str(K[c][1, 1]), centerPointU=str(K[c][0, 2]), centerPointV=str(K[c][1, 2]),
                         # skew='0.000000', radialDistortion1=str(D[c][0]), radialDistortion2=str(D[c][1]),
                         # radialDistortion3='0.000000', tangentalDistortion1=str(D[c][2]), tangentalDistortion2=str(D[c][3]))

    etree.ElementTree(root).write(qca_path, xml_declaration=True, pretty_print=True)

    with open(qca_path, 'r') as f:
        txt = f.read().replace('qtmversion', 'qtm-version', 1).replace('stddev', 'std-dev', 1)\
                      .replace('minmaxdiff', 'min-max-diff', 1).replace('pointcount', 'point-count')\
                      .replace('avgresidual', 'avg-residual')
    with open(qca_path, 'w') as f:
        f.write(txt)


def calib_toml_to_qca_func(input_file, output_file=None,filename_convention=2, binning_factor=1, pixel_size=5.54e-3):
    if output_file is None:
        output_file = input_file.replace('.toml', '.qca.txt')
    ...
    C, S, D, K, R, T = read_toml(input_file)
    R = [np.array(cv2.Rodrigues(r)[0]) for r in R]
    T = np.array(T) * 1000  # Convert meters to millimeters

    RT = [rotate_cam(r, t, ang_x=np.pi) for r, t in zip(R, T)]
    R = [rt[0] for rt in RT]
    T = [rt[1] for rt in RT]
    ...
    qca_write(output_file, C, S, D, K, R, T, binning_factor, pixel_size,filename_convention)
...
def run_calibration(source_Videos_folder,target_calibration_dir, VideoExtrisic_destination_root, project_dir, selected_idx=None, filename_convention=1):
    ...
    trials = group_videos_by_trial(video_files,filename_convention)
    # If no selection index provided, prompt the user with a popup
    if selected_idx == 0:
        # Setup hidden root window for Tkinter popup
        root = tk.Tk()
        root.withdraw()

        # Setup popup window to select trial and Config.toml
        popup = tk.Toplevel()
        popup.title("Calibration Setup")
        popup.geometry("500x350")
        popup.grab_set()  # Make popup modal
        
        tk.Label(popup, text="Select Calibration Trial:").pack(pady=10)
        
        trial_var = tk.StringVar()
        trial_options = []
        
        for idx, trial in enumerate(trials):
            trial_name = parse_timestamp(trial[0][0], filename_convention).strftime("%Y%m%d_%H%M%S")
            text = f"[{idx}] Trial {trial_name} ({len(trial)} video(s))"
            trial_options.append(text)
        
        # Dropdown menu
        trial_dropdown = ttk.Combobox(popup, textvariable=trial_var, values=trial_options, state="readonly", width=60)
        trial_dropdown.pack(pady=10)
        
        # Config.toml (Object coordinates)
        tk.Label(popup, text="Select Config.toml file (which contains the Objects coordinates):").pack(pady=(10, 0))
        config_frame = tk.Frame(popup)
        config_frame.pack(pady=5)
        config_path_var = tk.StringVar()
        config_entry = ttk.Entry(config_frame, textvariable=config_path_var, width=50)
        config_entry.pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(config_frame, text="Browse", command=lambda: config_path_var.set(
            filedialog.askopenfilename(filetypes=[("TOML files", "*.toml")])
        )).pack(side=tk.LEFT)
        
        # Intrinsic Calibration file
        tk.Label(popup, text="Select Intrinsic Calibration TOML file (might be Calib_scene.toml):").pack(pady=(15, 0))
        intrinsic_frame = tk.Frame(popup)
        intrinsic_frame.pack(pady=5)
        intrinsic_path_var = tk.StringVar()
        intrinsic_entry = ttk.Entry(intrinsic_frame, textvariable=intrinsic_path_var, width=50)
        intrinsic_entry.pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(intrinsic_frame, text="Browse", command=lambda: intrinsic_path_var.set(
            filedialog.askopenfilename(filetypes=[("TOML files", "*.toml")])
        )).pack(side=tk.LEFT)
        
        def on_confirm():
            if not trial_var.get():
                messagebox.showwarning("Selection Required", "Please select a trial.")
                return
            if not config_path_var.get() or not os.path.isfile(config_path_var.get()):
                messagebox.showwarning("File Required", "Please select a valid Config.toml file.")
                return
            if not intrinsic_path_var.get() or not os.path.isfile(intrinsic_path_var.get()):
                messagebox.showwarning("File Required", "Please select a valid Calib.toml file.")
                return
        
            # For now we just save and use it later
            popup.destroy()
        
        ttk.Button(popup, text="Confirm", command=on_confirm).pack(pady=20)
        popup.wait_window()
        if not trial_var.get() or not config_path_var.get() or not intrinsic_path_var.get():
            print("User cancelled the calibration selection.")
            return

        selected_idx = int(trial_var.get().split(']')[0][1:])
        config_path = config_path_var.get()
        calib_file_path = intrinsic_path_var.get()
    else:
    ...
    for video_path, _ in calibration_trial:
        cam_num = get_camera_number(video_path,filename_convention)
        if cam_num:
    ...
    ret, C, S, D, K, R, T = calib_calc_fun(calib_dir, intrinsics_config, extrinsics_config, calib_file_path, filename_convention)
    print("Calibration complete.")

    print("Converting calibration file from .toml to .qca")
    
    
    # toml_file = os.path.join(calib_dir, 'Calib_scene.toml')
    
    # toml_write(toml_file, C, S, D, K, R, T)
    
    output_toml_file = os.path.join(target_calibration_dir, 'Calib_scene.toml')
    output_qca_file = output_toml_file.replace('.toml', '.qca.txt')
    
    toml_write(output_toml_file, C, S, D, K, R, T)
    
    calib_toml_to_qca_func(
        input_file=output_toml_file,
        output_file=output_qca_file,
        filename_convention=filename_convention
    )
    
    # Recap message
    recap_calibrate(ret, output_toml_file)
    
    
# def run_calibration(source_Videos_folder, VideoExtrisic_destination_root, project_dir, selected_idx=0):
#     if os.path.exists(VideoExtrisic_destination_root):
#         for item in os.listdir(VideoExtrisic_destination_root):
#             item_path = os.path.join(VideoExtrisic_destination_root, item)
#             try:
...
#     if selected_idx >= len(trials):
#         raise IndexError("Invalid calibration trial index")
...
#     print("Converting calibration file from .toml to .qca")
#     toml_file = os.path.join(calib_dir, 'Calib_scene.toml')
#     output_qca_file = toml_file.replace('.toml', '.qca.txt')
#     calib_toml_to_qca_func(toml_file, output_qca_file)
#     print(f"Calibration saved to {output_qca_file}")


# if __name__ == "__main__":
    
    # source_Videos_folder = r"C:\videos\DCIM\1"
