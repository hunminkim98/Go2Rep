...
def group_videos_by_trial(video_files, time_tolerance=8):
...
def process_videos(source_dir, target_dir, sync_json=None, convert=False,  use_sync_file=False, format_choice='mp4', filename_convention=1):
    ...
    video_paths = [os.path.normpath(os.path.join(source_dir, f)) for f in video_files]
    ...
    for trial_index, trial in enumerate(trials, start=1):
        ...
        for video_path, _ in trial:
            ...
            if filename_convention == 1:  # GoPro
                match = re.search(r'GoPro(\d+)', video_name)
                if match:
                    camera_name = f"GoPro{match.group(1)}"
                else:
                    camera_name = "UnknownGoPro"
            elif filename_convention == 2:  # CAMERA
        ...
        if use_sync_file and sync_json:
            ...
            if trial_id in sync_json:
                ...
                # Ensure start_frame doesn't go negative 
                print("🔍 Validating start frame range across all videos...\n")
                if sync_data['start_frame_on_reference_video']+min_offset < 0:
                    sync_data['start_frame_on_reference_video'] += abs(min_offset)
                    print(f"🔄 Adjusted starting frame by +{abs(min_offset)} to compensate for negative offsets.")    
                # Step 1: Pre-calculate max valid end frame across all videos
                print("🔍 Validating end_frame range across all videos...\n")
                max_valid_end_frame = float('inf')
                for video_path, _ in trial:
                    if video_path in offsets:
                        try:
                            clip = VideoFileClip(video_path)
                            offset = offsets[video_path]
                            duration_frames = int(clip.fps * clip.duration)
                            max_end = duration_frames - offset  # max usable frame on reference scale
                            max_valid_end_frame = min(max_valid_end_frame, max_end)
                            clip.close()
                        except Exception as e:
                            print(f"❌ Could not check duration for {video_path}: {e}")
                
                # Step 2: Adjust end_frame_on_reference_video globally if needed
                if sync_data['end_frame_on_reference_video'] > max_valid_end_frame:
                    print(f"⚠️ Adjusted 'end_frame_on_reference_video' from {sync_data['end_frame_on_reference_video']} to {max_valid_end_frame} to fit all clips.")
                    sync_data['end_frame_on_reference_video'] = max_valid_end_frame
                        ...
                        try:
                            ...
                            if filename_convention == 1:  # GoPro
                                ...
                                if match:
                                    ...
                                    camera_name = "UnknownGoPro"
                                    print(f"⚠️ Warning: Could not find GoPro camera number in '{video_name}'. Reselect the file name convention or video will be stored in 'UnknownGoPro' folder.")
                            elif filename_convention == 2:  # CAMERA
                                match = re.search(r'CAMERA(\d+)', video_name)
                                if match:
                                    camera_name = f"CAMERA{match.group(1)}"
                                    ...
                                    camera_name = "UnknownCAMERA"
                                    print(f"⚠️ Warning: Could not find CAMERA number in '{video_name}'. Reselect the file name convention or video will be stored in 'UnknownCAMERA' folder.")
                            else:
                                camera_name = "Unknown"
                                print(f"⚠️ Warning: Could not find CAMERA number in '{video_name}'. Storing in 'UnknownCAMERA' folder.")
                            ...
                            #   camera_name = f"CAMERA{match.group(1)}"
                                ...
                                if filename_convention == 1:
                                    video_name_2Save = re.sub(r'.*-(GoPro\d+)-.*', r'\1', video_name)
                                else:
            ...
            for clip, output_path, _ in video_data:
                ...
                # Adjust output filename based on filename convention
                if filename_convention == 1:  # GoPro
                    output_path = re.sub(r'[^\\]+-(GoPro\d+)-[^\\]+\.mp4$', r'\1.mp4', output_path)
                elif filename_convention == 2:  # CAMERA
...
# Go2Rep/tools/Theia_classifier.py

def run_theia_classification(source_dir, target_dir, use_sync=False, format_choice="mp4", filename_convention=1):
    sync_json = None
    # Create Classified directory if it doesn't exist
    classification_dir = os.path.join(target_dir, "Classified")
    os.makedirs(classification_dir, exist_ok=True)
    # Use Synchronisation/output.json as the sync file path
    sync_file_path = os.path.join(target_dir, "Synchronisation", "output.json")
    # Load synchronization file if needed
    if use_sync:
        if os.path.exists(sync_file_path):
            sync_json = load_synchronization_json(sync_file_path)
            print("✅ Sync file Was found. Continuing Using sync.")
        else:
            print("⚠️ Sync file not found. Continuing without sync.")
            use_sync = False  # Fail-safe

    # Prepare arguments
    sync_json_arg = sync_json if use_sync else None
    
    # Call the function
    process_videos(
        source_dir,
        classification_dir,
        sync_json=sync_json_arg,
        convert=True,
        format_choice=format_choice,
        use_sync_file=use_sync,
